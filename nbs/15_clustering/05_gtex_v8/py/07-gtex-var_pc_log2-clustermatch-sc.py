...
# %% [markdown] tags=[]
# It runs a spectral clustering algorithm on the similarity matrix generated by the correlation method specified below (under `Settings`). It saves the set of clustering solutions (called "ensemble") into a pandas dataframe.
...
# %% tags=[]
input_files = sorted(
    list(INPUT_DIR.glob(f"*-{GENE_SELECTION_STRATEGY}-{CORRELATION_METHOD_NAME}.pkl"))
)
input_files = [
    f
    for f in input_files
    if any(f"gtex_v8_data_{tn}-" in f.name for tn in tissue_names)
]
